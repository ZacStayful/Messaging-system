        ...
        {result?.ok ? (
          <section className={card}>
            ...
            <div className="mt-4 flex gap-2">
              ...
              <Link
                href="/home"
                className="flex h-11 items-center rounded-lg border border-input-border px-4 text-[15px] font-semibold text-ink no-underline hover:bg-hover"
              >
                Go to Home
              </Link>
            </div>
          ...
          <form action={submit} className="flex flex-col gap-4">
            <section className={`${card} flex flex-col gap-3`}>
              <h2 className="text-[16px] font-bold">Who</h2>
            ...
            <section className={`${card} flex flex-col gap-3`}>
              <h2 className="text-[16px] font-bold">Groups</h2>
              <label className="flex items-start gap-3 rounded-lg border border-line px-3.5 py-3">
                ...
                />
                <span>
                  <span className="block text-[15px] font-semibold">Create a new group for this customer</span>
                  <span className="block text-[14px] text-muted">
                    Named from their full name, e.g. <code>jason-beckhurst</code>. You are added automatically.
                  </span>
                </span>
              </label>
              <div className="text-[14px] font-semibold">Also add to existing groups</div>
              <input
                value={filter}
              ...
              <div className="scroll-thin max-h-56 overflow-y-auto rounded-lg border border-line">
                {visible.length === 0 && <p className="px-3.5 py-3 text-[14px] text-muted">No groups match.</p>}
                {visible.map((g) => (
                  <label
                    key={g.id}
                  ...
                  >
                    ...
                    {g.topic && <span className="truncate text-[13px] text-muted">{g.topic}</span>}
                  </label>
                ))}
              </div>
            </section>

            <section className={`${card} text-[14px] text-muted`}>
